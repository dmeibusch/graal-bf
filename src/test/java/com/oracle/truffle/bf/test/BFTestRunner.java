...
import com.oracle.truffle.bf.test.BFTestRunner.TestCase;
...
public class BFTestRunner extends ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".bf";
    ...
    public BFTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        BFTestSuite suite = c.getAnnotation(BFTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", BFTestSuite.class.getSimpleName(), c.getName(), BFTestRunner.class.getSimpleName()));
        }

        String[] paths = suite.value();

        Class<?> testCaseDirectory = c;
        if (suite.testCaseDirectory() != BFTestSuite.class) {
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        BFTestRunner suite = new BFTestRunner(testClass);
        if (args.length > 0) {
